...
```{r r, fig.height = 6, fig.width = 8, fig.align = "center"}
library("RcppDynProg")

plot <- requireNamespace("rqdatatable", quietly = TRUE) &&
...
solve_with_penalty <- function(ycol, penalty) {
  n <- length(ycol)
  indices = seq_len(n)
  x <- const_costs(ycol, 1+numeric(n), indices)
  x <- x + penalty
...
if(plot) {
  d <- d %.>% 
    extend(., group_mean = mean(y_observed),
           partitionby = "group") %.>%
    as.data.frame(.)
...
```{r l, fig.height = 6, fig.width = 8, fig.align = "center"}
